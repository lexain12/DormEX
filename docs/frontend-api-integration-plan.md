...
### Текущая точка
- План 5.1–5.3 завершён и покрыт по коду.
- Дополнительный UX-hardening для нестабильного backend выполнен.
- В live-экранах фронтенда не осталось локальных sample-данных для уже интегрированных API-разделов.
...
- Удаление моков из интегрированных экранов:
  - `TaskDetail` больше не использует локальные `PRICE_HISTOGRAM` и `SAMPLE_TRANSACTIONS`;
  - аналитический блок на карточке задачи переведён на реальные category analytics из API;
  - список последних сделок заменён на controlled empty-state, так как такого endpoint в `API.md` нет;
  - fallback-бейджи в `Profile` больше не статические, а вычисляются из реальных данных пользователя.
  - `FeaturedCard` больше не использует sample fallback и статическое число сделок.
