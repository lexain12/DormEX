...
import { PAYMENT_LABELS, STATUS_LABELS, URGENCY_LABELS } from "@/lib/data";
...
const TaskDetail = () => {
  ...
  const [selectedOfferForCounter, setSelectedOfferForCounter] = useState<
    number | null
  >(null);
  ...
  const [counterPaymentType, setCounterPaymentType] =
    useState<ApiPaymentType>("fixed_price");
  ...
  const [editOfferPaymentType, setEditOfferPaymentType] =
    useState<ApiPaymentType>("negotiable");
  const [editOfferPriceValue, setEditOfferPriceValue] = useState("");
  const [editOfferBarterDescription, setEditOfferBarterDescription] =
    useState("");
  ...
  const isAssignedPerformer =
    taskQuery.data?.accepted_offer?.performer?.id === user?.id;
  ...
  const counterOffersQuery = useQuery({
    ...
    queryFn: () =>
      offersService.listCounterOffers(selectedOfferForCounter as number),
    enabled: actionModal === "counter" && Boolean(selectedOfferForCounter),
    refetchInterval:
      actionModal === "counter" && Boolean(selectedOfferForCounter)
        ? 60_000
        : false,
  });
  ...
  const analyticsQuery = useQuery({
    ...
    enabled:
      taskQuery.isSuccess &&
      Boolean(analyticsCategory) &&
      !taskQuery.data?.analytics,
  });

  const task = taskQuery.data ? mapTaskDtoToUi(taskQuery.data) : null;
  const categoryAnalytics =
    taskQuery.data?.analytics ?? analyticsQuery.data ?? null;
  const analyticsHistogram = categoryAnalytics?.price_histogram ?? [];
  const analyticsMaxCount = Math.max(
    ...analyticsHistogram.map((entry) => entry.count),
    0,
  );
  ...
  const chatIdFromTask =
    typeof taskQuery.data?.chat_id === "number" ? taskQuery.data.chat_id : null;
  const chatIdFromList =
    chatsQuery.data?.find((chat) => chat.task_id === numericTaskId)?.id ?? null;
  const chatId = chatIdFromTask ?? chatIdFromList;
  const canOpenChat = Boolean(chatId && isAssignedPerformer);
    ...
    onMessage: (message: { task?: TaskDetailDto; offers?: OfferDto[] }) => {
      ...
      if (message.offers) {
        queryClient.setQueryData(
          queryKeys.offers(numericTaskId),
          message.offers,
        );
      }
  ...
  useRealtimeChannel({
    ...
    onMessage: (message: { items?: CounterOfferDto[] }) => {
      ...
      if (message.items) {
        queryClient.setQueryData(
          queryKeys.counterOffers(selectedOfferForCounter),
          message.items,
        );
      }
      ...
      void Promise.all([
        queryClient.refetchQueries({
          queryKey: queryKeys.counterOffers(selectedOfferForCounter),
          type: "active",
        }),
        queryClient.refetchQueries({
          queryKey: queryKeys.offers(numericTaskId),
  ...
  const createOfferMutation = useMutation({
    mutationFn: (payload: {
      message: string;
      payment_type: "fixed_price" | "negotiable";
  ...
  const sendMessageMutation = useMutation({
    mutationFn: (payload: { chatId: number; body: string }) =>
      chatsService.sendMessage(payload.chatId, payload.body),
    onSuccess: async () => {
      if (chatId) {
    ...
    onError: (error) => {
      setCounterError(
        error instanceof Error
          ? error.message
          : "Не удалось отправить контрпредложение",
      );
    },
  });
  ...
  const openDisputeMutation = useMutation({
    mutationFn: (comment: string) =>
      tasksService.openDispute(numericTaskId, comment),
    onSuccess: async () => {
      await Promise.all([
        queryClient.refetchQueries({
          queryKey: queryKeys.task(numericTaskId),
  ...
  const createReviewMutation = useMutation({
    mutationFn: (payload: { rating: number; comment: string | null }) =>
      tasksService.createReview(numericTaskId, payload),
    onSuccess: async () => {
      await Promise.all([
        queryClient.refetchQueries({
          queryKey: queryKeys.task(numericTaskId),
        ...
        queryClient.refetchQueries({
          queryKey: queryKeys.taskReviews(numericTaskId),
          type: "active",
        }),
        queryClient.refetchQueries({
          queryKey: queryKeys.userProfile(user?.id ?? 0),
          type: "active",
        }),
        queryClient.refetchQueries({
  ...
  const handlePriceSubmit = async (event: FormEvent) => {
    ...
    const payload = [priceComment.trim(), `${normalizedPrice} ₽`]
      .filter(Boolean)
      .join(" · ");
    ...
    try {
      ...
      setPriceError(
        error instanceof Error ? error.message : "Не удалось отправить цену",
      );
    }
  };
  ...
  const handleMessageSubmit = async (event: FormEvent) => {
    ...
    if (!chatId) {
      setMessageError(
        "Чат ещё не создан. Он появится после выбора исполнителя.",
      );
      return;
    }
  ...
  const handleCounterOfferSubmit = async (event: FormEvent) => {
    ...
    if (
      ...
      (!counterPriceValue ||
        Number.isNaN(normalizedCounterPrice) ||
        normalizedCounterPrice <= 0)
    ) {
    ...
    await createCounterOfferMutation.mutateAsync({
      ...
      price_amount:
        counterPaymentType === "fixed_price" ? normalizedCounterPrice : null,
      barter_description:
        counterPaymentType === "barter" ? normalizedCounterBarter : null,
    });
  };
  ...
  const handleEditOfferSubmit = async (event: FormEvent) => {
    ...
    await updateOfferMutation.mutateAsync({
      ...
      price_amount:
        editOfferPaymentType === "fixed_price" ? normalizedPrice : null,
      barter_description:
        editOfferPaymentType === "barter" ? normalizedBarter : null,
    });
  };
  ...
  const canRespond =
    taskQuery.data?.can_respond ??
    (task?.status === "open" || task?.status === "offers");
  const canChoosePerformer =
    taskQuery.data?.can_choose_performer ??
    (isTaskOwner && task?.status === "offers");
  const canManageProgress = Boolean(
    (isTaskOwner || isAssignedPerformer) && task?.status === "progress",
  );
  const completionStatus =
    taskQuery.data?.completion_confirmation_status ?? null;
  const completionConfirmedByMe = Boolean(
    taskQuery.data?.completion_confirmed_by_me,
  );
  ...
  const isAwaitingOtherPartyConfirmation =
    task?.status === "progress" &&
  ...
  const ownPendingOffer =
    offersQuery.data?.find(
      (offer) => offer.performer?.id === user?.id && offer.status === "pending",
    ) ?? null;
  ...
  useEffect(() => {
    ...
    if (autoPanel === "chat" && chatId && isAssignedPerformer) {
    ...
    if (task.status === "open") {
      if (isTaskOwner) {
        return {
          ...
          description:
            "Как только кто-то предложит свои условия, здесь появится список кандидатов.",
        };
      }

      return {
        title: ownPendingOffer
          ? "Ваш отклик уже отправлен"
          : "Можно откликнуться",
    ...
    if (task.status === "offers") {
      if (isTaskOwner) {
        return {
          ...
          description:
            "Сравните отклики, откройте переговоры и выберите исполнителя, когда условия устроят.",
        };
      }
      ...
      if (ownPendingOffer) {
        return {
          ...
          description:
            "Заказчик уже видит ваш отклик. Если потребуется, он сможет открыть переговоры по условиям.",
        };
      }
    ...
    if (task.status === "progress") {
      if (hasOpenDispute) {
        return {
          ...
          description:
            "Сделка приостановлена до урегулирования спорной ситуации между сторонами.",
        };
      }
      ...
      return {
        ...
        description:
          "Исполнитель выбран, поэтому новые отклики больше не принимаются.",
      };
    }
    ...
    if (task.status === "done") {
      if (reviewSummary?.pending_by_me) {
        return {
          ...
          description:
            "Оставьте короткую оценку второй стороне, чтобы сделка полностью закрылась в истории.",
        };
      }
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center px-4 sm:px-6">
        <div className="card-surface p-6 max-w-md w-full">
          ...
          <Link
            to="/"
            className="inline-block mt-3 text-sm text-primary hover:text-primary/80"
          >
            Вернуться к ленте
          </Link>
        </div>
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-[1400px] mx-auto px-4 py-4 sm:px-6 sm:py-6">
        <Link
          to="/"
          className="mb-4 inline-flex items-center gap-1.5 text-sm text-muted-foreground transition-colors hover:text-foreground sm:mb-5"
        >
            ...
            <div className="flex-1 min-w-0 space-y-5">
              <div className="card-surface p-4 sm:p-6">
                ...
                <div className="grid grid-cols-1 gap-4 rounded-lg bg-secondary p-4 sm:grid-cols-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm font-semibold text-foreground">
                      {task.price ? `${task.price} ₽` : "По договорённости"}
                    </div>
                  </div>
                  <div>
                    <div className="text-[11px] text-muted-foreground mb-0.5">
                ...
                {roleStageSummary && (
                  <div className="mt-5 rounded-lg border border-border bg-secondary/40 p-4">
                    <div className="text-sm font-semibold text-foreground">
                      {roleStageSummary.title}
                    </div>
                    <div className="mt-1 text-sm text-muted-foreground">
                      {roleStageSummary.description}
                    </div>
                  </div>
                )}
              ...
              <div className="card-surface p-4 sm:p-6">
                <h3 className="font-semibold text-sm text-foreground mb-4">
                  Аналитика категории: {task.categoryIcon} История сделок
                </h3>
                ...
                {isAnalyticsLoading ? (
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
                      {Array.from({ length: 4 }).map((_, index) => (
                        <div
                          key={index}
                          className="h-20 rounded-lg bg-secondary animate-pulse"
                        />
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="p-4 rounded-lg border border-border bg-secondary/40">
                    <div className="text-sm text-foreground">
                      Не удалось загрузить аналитику категории.
                    </div>
                    <button
                      type="button"
                  ...
                  <>
                    {analyticsHistogram.length > 0 ? (
                      ...
                      <div className="h-48 mb-4 rounded-lg border border-border bg-secondary/40 flex items-center justify-center text-sm text-muted-foreground">
                        По этой категории пока нет гистограммы завершённых
                        сделок.
                      </div>
                    )}
                      ...
                      {[
                        {
                          ...
                          value: `${categoryAnalytics.median_price_amount} ₽`,
                        },
                        {
                          label: "Средняя цена",
                          value: `${categoryAnalytics.avg_price_amount} ₽`,
                        },
                        {
                          label: "Диапазон",
                          value: `${categoryAnalytics.min_price_amount}–${categoryAnalytics.max_price_amount} ₽`,
                        },
                        {
                          label: "Среднее время",
                          value: `${Math.round((categoryAnalytics.avg_completion_minutes / 60) * 10) / 10} часа`,
                        },
                      ].map((s) => (
                        <div
                          key={s.label}
                    ...
                    <div className="mt-5 pt-5 border-t border-border">
                      ...
                      <div className="p-3 rounded-lg border border-border bg-secondary/50 text-sm text-muted-foreground">
                        API пока возвращает только агрегированную аналитику по
                        категории. Список отдельных последних сделок для этой
                        карточки ещё недоступен.
                      </div>
                    </div>
                  </>
                ) : (
                ...
                reviewSummary.my_role && (
                  <div className="card-surface p-4 sm:p-6">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">
                          После закрытия сделки обе стороны могут оставить по
                          одному отзыву друг о друге.
                        </p>
                      </div>
                    ...
                    <div className="mt-4 grid grid-cols-1 gap-3 lg:grid-cols-2">
                        ...
                        {reviewSummary.my_review ? (
                          <>
                            <div className="mt-2 flex items-center gap-1">
                              {Array.from({
                                length: reviewSummary.my_review.rating,
                              }).map((_, index) => (
                                <Star
                                  key={index}
                            ...
                            <div className="mt-2 text-sm text-foreground">
                              {reviewSummary.my_review.comment ||
                                "Без комментария"}
                            </div>
                          </>
                        ) : (
                      ...
                      <div className="rounded-lg border border-border bg-secondary/40 p-4">
                        <div className="text-xs uppercase tracking-[0.14em] text-muted-foreground">
                          Отзыв второй стороны
                        </div>
                        {reviewSummary.counterpart_review ? (
                          <>
                            <div className="mt-2 flex items-center gap-1">
                              {Array.from({
                                length: reviewSummary.counterpart_review.rating,
                              }).map((_, index) => (
                                <Star
                                  key={index}
                                  className="w-4 h-4 text-warning fill-warning"
                            ...
                            <div className="mt-2 text-sm text-foreground">
                              {reviewSummary.counterpart_review.comment ||
                                "Без комментария"}
                            </div>
                          </>
                        ) : (
              ...
              <div className="card-surface space-y-3 p-5 xl:sticky xl:top-20">
                ...
                {ownPendingOffer && (
                  <div className="rounded-lg border border-border bg-secondary/40 p-3 text-sm text-muted-foreground">
                    Ваш отклик уже отправлен. Заказчик увидит его в списке и
                    сможет открыть переговоры по условиям.
                  </div>
                )}
                  ...
                  onClick={() => {
                    if (!isAssignedPerformer) {
                      toast({
                        ...
                        description:
                          "Чат доступен только выбранному исполнителю.",
                      });
                      return;
                    }
                ...
                {hasOpenDispute && (
                  <div className="rounded-lg border border-border bg-secondary/40 p-3 text-xs text-muted-foreground">
                    По задаче открыт спор. До его урегулирования подтверждение
                    завершения недоступно.
                  </div>
                )}
                ...
                {!offersQuery.isLoading && !offersQuery.isError && (
                  <div className="space-y-2">
                    {(offersQuery.data ?? []).slice(0, 5).map((offer) => {
                      const isOwnPendingOffer =
                        offer.performer?.id === user?.id &&
                        offer.status === "pending";
                      const canCounterOffer =
                        offer.status === "pending" &&
                        (isTaskOwner || offer.performer?.id === user?.id);

                      return (
                        <div
                          key={offer.id}
                          className="p-2.5 rounded-lg bg-secondary"
                        >
                          <div className="flex flex-wrap items-center justify-between gap-2">
                            ...
                            <span
                              className={`chip text-[10px] px-1.5 py-0.5 ${getOfferStatusClass(offer.status)}`}
                            >
                            ...
                            <div className="mt-2 flex flex-wrap items-center gap-3">
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  setEditOfferPriceValue(
                                    offer.price_amount
                                      ? String(offer.price_amount)
                                      : "",
                                  );
                                  setEditOfferBarterDescription(
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Опишите, как вы готовы выполнить задачу.
            </DialogDescription>
          </DialogHeader>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Согласуйте условия по выбранному отклику.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            <div className="max-h-44 overflow-y-auto rounded-lg border border-border bg-secondary/40 p-3 space-y-2">
              ...
              {!counterOffersQuery.isLoading &&
                ...
                (counterOffersQuery.data ?? []).length === 0 && (
                  <div className="text-xs text-muted-foreground">
                    История переговоров пока пуста.
                  </div>
                )}
              ...
              {!counterOffersQuery.isLoading &&
                !counterOffersQuery.isError &&
                (counterOffersQuery.data ?? []).map((counter) => {
                  const isOwn = counter.author_user_id === user?.id;
                  const isPendingForAction =
                    counter.status === "pending" && !isOwn;
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        ...
                        <span
                          className={`chip text-[10px] px-1.5 py-0.5 ${counter.status === "accepted" ? "status-done" : counter.status === "rejected" ? "status-cancelled" : "status-offers"}`}
                        >
                          {counter.status}
                        </span>
                      </div>
                      ...
                      {isPendingForAction && (
                        <div className="mt-2 flex flex-wrap items-center gap-2">
                          <button
                            type="button"
                            onClick={() =>
                              acceptCounterOfferMutation.mutate(counter.id)
                            }
                            className="text-[11px] text-primary hover:text-primary/80"
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              rejectCounterOfferMutation.mutate(counter.id)
                            }
                            className="text-[11px] text-muted-foreground hover:text-foreground"
            ...
            <form className="space-y-3" onSubmit={handleCounterOfferSubmit}>
              ...
              {counterError && (
                <p className="text-xs text-destructive">{counterError}</p>
              )}
              <button
                type="submit"
                className="w-full h-11 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 transition-colors"
              ...
              >
                {createCounterOfferMutation.isPending
                  ? "Отправляем..."
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Обновите сообщение и условия вашего отклика.
            </DialogDescription>
          </DialogHeader>
          ...
          <form className="space-y-3" onSubmit={handleEditOfferSubmit}>
            ...
            {editOfferError && (
              <p className="text-xs text-destructive">{editOfferError}</p>
            )}
            <button
              type="submit"
              className="w-full h-11 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 transition-colors"
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Чат доступен после выбора исполнителя.
            </DialogDescription>
          </DialogHeader>
          ...
          {chatId ? (
                    ...
                    return (
                      <div
                        ...
                        className={`rounded-lg p-2.5 text-xs ${
                          isOwn
                            ? "ml-2 bg-primary text-primary-foreground sm:ml-6"
                            : "mr-2 border border-border bg-card sm:mr-6"
                        }`}
                      >
            ...
            <div className="text-sm text-muted-foreground">
              Чат ещё не создан. Он будет доступен после назначения исполнителя
              по задаче.
            </div>
          )}
        </DialogContent>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Укажите причину отмены, она будет сохранена в истории.
            </DialogDescription>
          </DialogHeader>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Опишите проблему по выполнению задачи.
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="flex flex-wrap items-center gap-2">
                {Array.from({ length: 5 }).map((_, index) => {
                  ...
                  return (
                    ...
                    >
                      <Star
                        className={`h-4 w-4 ${active ? "fill-warning" : ""}`}
                      />
                    </button>
                  );
                })}
